...
shopper_service is a tiny web application made with Go 
...
$ helm upgrade -i my-release shopper_service/shopper_service 
```

The command deploys shopper_service on the Kubernetes cluster in the default namespace.
...
The following tables lists the configurable parameters of the shopper_service chart and their default values.
...
`image.repository` | `stefanprodan/shopper_service` | Image repository
...
`service.grpcService` | `shopper_service` | gPRC service name
...
```console
$ helm install my-release shopper_service/shopper_service \
...
```console
$ helm install my-release shopper_service/shopper_service -f values.yaml
```
