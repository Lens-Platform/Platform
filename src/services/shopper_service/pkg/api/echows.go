...
func (s *Server) echoWsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err != nil {
			s.logger.Error(err,"websocket upgrade error", zap.Error(err))
			return
		}
	}
	defer c.Close()
	...
	for {
		...
		if err != nil {
			if !strings.Contains(err.Error(), "close") {
				s.logger.Error(err,"websocket read error", zap.Error(err))
			}
			break
		}
		var response = struct {
	...
	for {
		...
		case msg := <-in:
			if err := ws.WriteJSON(msg); err != nil {
				if !strings.Contains(err.Error(), "close") {
					s.logger.Error(err, "websocket write error", zap.Error(err))
				}
				return
			}
		}
