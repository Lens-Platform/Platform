...
func (s *Server) cacheDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.logger.Error(err, "cache delete failed", zap.Error(err))
...
func (s *Server) cacheReadHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || !ok {
		s.logger.Error(err, "cache key not found", zap.String("key", key))
	...
	if err != nil {
		s.logger.Error(err,"cache get failed", zap.Error(err))
...
func (s *Server) startCachePool(ticker *time.Ticker, stopCh <-chan struct{}) {
	...
	setVersion := func() {
		...
		if _, err := conn.Do("SET", s.config.Hostname, version.VERSION, "EX", 60); err != nil {
			s.logger.Error(err,"cache server is offline", zap.Error(err), zap.String("server", s.config.CacheServer))
		}
		_ = conn.Close()
	}
