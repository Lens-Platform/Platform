...
func (s *Server) storeWriteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.logger.Error(err,"writing file failed", zap.Error(err), zap.String("file", path.Join(s.config.DataPath, hash)))
...
func (s *Server) storeReadHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.logger.Error(err, "reading file failed", zap.Error(err), zap.String("file", path.Join(s.config.DataPath, hash)))
