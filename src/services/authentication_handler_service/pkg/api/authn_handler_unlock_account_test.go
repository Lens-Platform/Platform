...
func TestUnLockAccountHandler(t *testing.T) {
	...
	for _, data := range testDataInfo {
		...
		opResult, err, rr := UnLockUserAccountRequestTestUtil(userId, authRes.Token, t)
...
func UnLockUserAccountRequestTestUtil(userId, token string, t *testing.T) (LockAccountResponse, error, *httptest.ResponseRecorder) {
	var result LockAccountResponse

	req, err := http.NewRequest("POST", "/v1/account/unlock/"+userId, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	srv := NewMockServer()
	handler := http.HandlerFunc(srv.unlockAccountHandler)
